...
static struct __attribute__((packed)) {
    ...
    union {
        ...
        struct {
            ...
            uint8_t  col;
            uint16_t keycode;
        } keymap_layer_overrides[(((((MATRIX_ROWS) + 1) / 2) * 2) * (((MATRIX_COLS + 1) / 2) * 2)) / ((sizeof(uint8_t) + sizeof(uint8_t) + sizeof(uint16_t)) / sizeof(uint16_t))];
        ...
        struct {
            ...
            uint8_t  clockwise;
            uint16_t keycode;
        } encodermap_layer_overrides[((((NUM_ENCODERS) + 1) / 2) * 2) * ((((NUM_DIRECTIONS) + 1) / 2) * 2) / ((sizeof(uint8_t) + sizeof(uint8_t) + sizeof(uint16_t)) / sizeof(uint16_t))];
#endif // defined(ENCODER_ENABLE) && defined(ENCODER_MAP_ENABLE)
    };
            ...
            if (sizeof(dynamic_keymap_scratch.encodermap_layer) <= sizeof(dynamic_keymap_scratch.encodermap_layer_overrides[0]) * dynamic_encodermap_altered_count[layer]) {
                ...
                for (int enc_id = 0; enc_id < NUM_ENCODERS; ++enc_id) {
                    for (int enc_dir = 0; enc_dir < NUM_DIRECTIONS; ++enc_dir) {
                        if (is_encodermap_altered(layer, enc_id, enc_dir)) {
                            dynamic_keymap_scratch.encodermap_layer_overrides[idx].encoder_id = enc_id;
                            dynamic_keymap_scratch.encodermap_layer_overrides[idx].clockwise  = enc_dir;
...
void nvm_dynamic_encodermap_load(void) {
    for (int layer = 0; layer < (sizeof(layer_state_t) * 8); ++layer) {
        ...
        if (dynamic_keymap_scratch.write_mode == 0) {
            ...
            for (int enc_id = 0; enc_id < NUM_ENCODERS; ++enc_id) {
                for (int enc_dir = 0; enc_dir < NUM_DIRECTIONS; ++enc_dir) {
                    nvm_dynamic_keymap_update_encoder(layer, enc_id, enc_dir, dynamic_keymap_scratch.encodermap_layer[enc_id][enc_dir]);
                }
            }
        } else {
            // overrides
            ...
            for (int j = 0; j < count; ++j) {
                ...
                uint8_t  clockwise  = dynamic_keymap_scratch.encodermap_layer_overrides[j].clockwise;
